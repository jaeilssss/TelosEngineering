...
const copyTree = (source: string, destination: string) => { rmSync(destination, { recursive: true, force: true }); mkdirSync(join(destination, ".."), { recursive: true }); cpSync(source, destination, { recursive: true }); };
...
function configureCodexHook(plugin: string) {
  const path = join(plugin, "hooks", "hooks.json"); const data = readJson(path);
  const handler = data.hooks.PostToolUse[0].hooks[0];
  handler.command = `${JSON.stringify(process.execPath)} "${"${PLUGIN_ROOT}"}/scripts/spec_gate.mjs"`;
  handler.commandWindows = `${JSON.stringify(process.execPath)} "%PLUGIN_ROOT%\\scripts\\spec_gate.mjs"`;
  writeJson(path, data);
}
function installCodex(home: string): string[] {
  ...
  configureCodexHook(plugin); installedVersion(plugin, "codex");
...
function configureClaudeHooks(plugin: string) {
  const path = join(plugin, "hooks", "hooks.json"); const data = readJson(path);
  for (const groups of Object.values(data.hooks ?? {}) as any[]) for (const group of groups) for (const hook of group.hooks ?? []) if (hook.type === "command" && hook.args) hook.command = process.execPath;
  writeJson(path, data);
}
function installClaude(home: string): string[] {
  const marketplace = join(home, ".telos", "claude-marketplace"); copyTree(join(resourceRoot, "claude-marketplace"), marketplace);
  ...
  const marketplacePath = join(marketplace, ".claude-plugin", "marketplace.json"); const catalog = readJson(marketplacePath); for (const entry of catalog.plugins ?? []) if (entry.name === "telos") entry.version = VERSION; writeJson(marketplacePath, catalog);
  configureClaudeHooks(plugin); installedVersion(plugin, "claude"); const messages = [`Claude marketplace copied to ${marketplace}`];
