# 명세 우선(Spec-First) 개발 규칙

이 규칙은 Telos 플러그인이 활성화된 모든 프로젝트에 적용된다.
핵심: *AI는 무엇이든 만들 수 있다. 어려운 건 무엇을 만들지 아는 것이다.*

## 절대 규칙

1. **스펙 없이 구현 금지.** 새 기능/모듈 구현 전, 해당 **프로젝트 루트**에 `SPEC.md`가
   있고 목표·제약·인수 기준(acceptance criteria)이 구체적이어야 한다. 없거나 모호하면
   코드를 쓰지 말고 `/telos:spec` 을 먼저 돌리도록 사용자에게 요청한다.
2. **항상 SPEC.md를 먼저 읽는다.** 구현·수정·평가 시작 시 프로젝트의 `SPEC.md` 를 읽고
   그 인수 기준을 기준점으로 삼는다.
3. **추측하지 말고 질문한다.** 모호함을 임의의 가정으로 메우지 않는다.

> 참고: 스펙 게이트 훅은 **차단이 아니라 경고**다. 일회성 스크립트나 탐색성 작업처럼
> 스펙이 과한 경우엔 무시하고 진행해도 된다. 규칙은 "제품 기능"에 적용한다는 의도다.

## 워크플로우 (우로보로스 4단계의 경량 버전)

```
/telos:spec   →  오케스트레이터가 현재 세션에서 질문 루프 진행 → ambiguity evaluator 통과 시 SPEC.md(계약) 생성
/telos:run    →  현재 Spec·저장소·실패 근거에 맞는 설치된 Capability를 발견·선택·실행
/telos:eval   →  3단계 게이트 (Mechanical → 조건부 semantic evaluator → (선택) Consensus)
반복    →  실패한 인수 기준을 근거로 Capability를 다시 라우팅하고 루프
```

`/telos:spec` 은 열린 질문이 모두 소진되면 `telos:spec-ambiguity-evaluator` 서브에이전트로
모호성 점검을 자동 수행하고, Ambiguity <= 0.2 일 때만 `frozen` 으로 전환한다.

Telos는 Spec과 final Eval을 소유한다. 설치된 스킬·플러그인·에이전트는 교체 가능한 Capability이며,
Telos는 현재 iteration에 필요한 것만 고른다.

## 3단계 평가 게이트 (비용 순서)

- **Stage 1 — Mechanical ($0):** 테스트·린트·타입체크·빌드. 통과 못 하면 여기서 멈춘다.
  LLM 토큰을 쓰기 전에 기계가 먼저 거른다.
- **Stage 2 — Semantic:** 작은 변경은 현재 세션 2차 검토로 평가하고, 고위험이거나 불확실성이 크면 `telos:spec-evaluator` 서브에이전트로 각 인수 기준 충족 여부를 근거와 함께 판정.
- **Stage 3 — Consensus (선택):** 고위험이거나 Stage 2가 불확실할 때만 다른 모델로 교차 검증.
