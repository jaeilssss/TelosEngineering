...
Telos는 명령 진입점을 얇은 오케스트레이터로 봅니다. 기본값은 현재 세션에서 작업을 이어가는 것이고, 서브 에이전트는 노이즈가 큰 탐색을 격리하거나 독립적인 평가 패스가 실제로 필요할 때만 사용합니다. 이 경로를 지원하지 않는 surface에서는 현재 세션으로 fallback 하되, spec/eval 게이트 자체는 건너뛰지 않습니다.
...
Telos는 다음 구조를 기본값으로 둡니다.

- 명령 진입점의 얇은 오케스트레이터
- spec 모호성 점검이나 좁은 저장소 탐색에 쓰는 선택적 worker
- 넓거나 노이즈가 큰 구현 탐색에 쓰는 impl worker
- 필요할 때 구현 워커와 독립적으로 동작하는 eval worker

오케스트레이터의 역할은 상태 전이와 handoff 관리이지, 구현이나 평가 판단을 다시 직접 떠안는 것이 아닙니다.
...
spec 흐름은 집중된 질문으로 요구사항을 정리하고 `SPEC.md`를 채운 뒤, 모호성 점검을 통과했을 때만 상태를 `frozen`으로 바꿉니다. 이때 예상 변경 범위, 프로젝트별 검증 명령, 해당 위험 검사, frozen 시점의 Git baseline도 기록합니다. 작고 국소적인 작업은 `$spec quick`으로 같은 계약을 짧게 만들 수 있지만, 고위험 작업은 일반 인터뷰를 사용합니다. 질문 루프는 기본적으로 메인 세션에 두고, 서브 에이전트는 좁고 노이즈가 큰 탐색이나 마지막 모호성 점검에만 사용합니다.
...
구현 흐름은 먼저 `SPEC.md`로 1차 범위와 리스크를 읽고, 이어서 실제 코드베이스를 확인해 그 판단을 보정합니다. 변경을 인수 기준에 매핑하고 선언된 범위를 벗어나거나 baseline이 오래된 경우 범위를 넓히기 전에 알립니다. 변경이 작고 국소적이면 메인 세션에서 바로 구현하고, 탐색 범위나 영향 반경이 커서 메인 컨텍스트에 저가치 탐색 로그가 많이 쌓일 때만 impl worker를 사용합니다.
...
1. 기록된 프로젝트별 검증 명령과 기계 검증
2. 변경 범위와 인수 기준·선택된 위험 기반 의미 평가
3. 회피 가능한 복잡성을 찾는 선택적 report-only lean pass
4. 고위험 또는 불확실한 경우 선택적 합의 검증

의미 평가는 구현 워커와 독립적으로 수행되는 편이 바람직하며, 구현 의도보다 증거를 우선합니다. 다만 변경이 작고 저위험이며 근거가 이미 충분히 압축돼 있으면 현재 세션에서 직접 평가해도 됩니다.
