...
spec 흐름은 집중된 질문으로 요구사항을 정리하고 `SPEC.md`를 채운 뒤, 모호성 점검을 통과했을 때만 상태를 `frozen`으로 바꿉니다. 질문 루프는 기본적으로 메인 세션에 두고, 서브 에이전트는 좁고 노이즈가 큰 탐색이나 마지막 모호성 점검에만 사용합니다.
...
구현 흐름은 먼저 `SPEC.md`로 1차 범위와 리스크를 읽고, 이어서 실제 코드베이스를 확인해 그 판단을 보정합니다. 변경이 작고 국소적이면 메인 세션에서 바로 구현하고, 탐색 범위나 영향 반경이 커서 메인 컨텍스트에 저가치 탐색 로그가 많이 쌓일 때만 impl worker를 사용합니다.
