...
기본적으로 Telos는 명령 진입점을 얇은 오케스트레이터로 보고, surface가 이를 자연스럽게 지원하면 `spec`, `impl`, `eval`을 fresh worker agent로 실행하는 쪽을 선호합니다. 이 경로가 깔끔하지 않은 surface에서는 현재 세션으로 fallback 하되, spec/eval 게이트 자체는 건너뛰지 않습니다.
...
- 명령 진입점의 얇은 오케스트레이터
- 요구사항 정리를 담당하는 spec worker
- frozen spec에서 시작하는 impl worker
- 구현 워커와 독립된 eval worker
...
spec 흐름은 집중된 질문으로 요구사항을 정리하고 `SPEC.md`를 채운 뒤, 모호성 점검을 통과했을 때만 상태를 `frozen`으로 바꿉니다. worker agent를 쓸 수 있으면 fresh spec worker를 우선 사용하고, 사용자 답변은 최소한으로만 재해석해 전달합니다.
...
worker agent를 쓸 수 있으면, 구현은 긴 요구사항 대화 이력 대신 `SPEC.md`와 관련 코드만으로 시작하는 fresh impl worker를 우선 사용합니다.
...
의미 평가는 구현 워커와 독립적으로 수행되어야 하며, 구현 의도보다 증거를 우선합니다.
